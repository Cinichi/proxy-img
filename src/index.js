// 🚀 Bandwidth Hero Cloudflare Worker v4.1
// ✅ Mangabuddy / Mangapill / Hentaifox / NHentai fixed
// ✅ Dual referer retry for Mangabuddy CDNs
// ✅ Works with Tachiyomi / Bandwidth Hero
// ✅ wsrv.nl + Direct fallback + KV stats
...
function getRefererForHost(hostname) {
  ...
  // 🔹 Mangabuddy numbered CDNs (s8.mbcdnsah.org, s21.mbcdnsav.org, etc.)
  ...
  // 🔹 Detective Conan / Mangapill
  if (host.includes("readdetectiveconan.com")) return "https://mangapill.com/";
  if (host.includes("mangapill.com")) return "https://mangapill.com/";
...
async function handleImageRequest(request, env, ctx) {
  ...
  if (!response.ok || !(response.headers.get("content-type") || "").includes("image/")) {
    ...
    // 🔁 Attempt 3: Mangabuddy backup referer
    if (
      response.status === 403 &&
      (parsedTarget.hostname.includes("mbcdnsah") ||
        parsedTarget.hostname.includes("mbcdnsav"))
    ) {
...
function getWebInterface() {
  return new Response(
    `<!DOCTYPE html><html><body style="font-family:sans-serif;padding:40px;">
<h2>⚡ Bandwidth Hero Proxy v4.1</h2>
